...
function normalizeTrailingCommas(tokens) {
...
function parseEnableConditions(tokens, argumentNames) {
  const conditions = [];
  const remaining = [];
  let error = '';
  for (const token of tokens) {
    const match = token.value.match(/^enable=(.+)$/);
    if (!match) {
    ...
    for (const part of match[1].split('&')) {
      const term = part.match(/^(!?)\{([A-Za-z_][A-Za-z0-9_]*)\}$/);
      if (!term) {
        error = `无法识别 enable 表达式 ${part}`;
        break;
      }
      const negated = term[1] === '!';
      const name = term[2];
      if (!argumentNames.has(name)) {
        error = `enable 参数 {${name}} 未在 [Argument] 中定义`;
        break;
      }
      terms.push(`\${${name}} ${negated ? '== false' : '== true'}`);
    }
    if (error) {
      break;
    }
  ...
  return {conditions, tokens: remaining, error};
}
...
function convertLegacyLine(line, lineNumber, captureName, argumentNames) {
  ...
  const normalized = normalizeTrailingCommas(tokenized.tokens);
  const enable = parseEnableConditions(normalized, argumentNames);
  if (enable.error) {
    return {
      line,
      issue: {
        line: lineNumber,
        level: 'error',
        message: `${enable.error}，已保留原文`,
      },
    };
  }

  const tokens = enable.tokens;
  if (tokens.length < 2) {
    return {
      line,
      issue: {
  ...
  if (action === 'header' || action === '302' || action === '307') {
    ...
    const enableCondition = enable.conditions.length
      ? ` && ${enable.conditions.join(' && ')}`
      : '';
    const method =
      action === 'header'
        ? 'url.replace'
    ...
    const actionText =
      action === 'header'
        ...
        : method;
    return {
      line: `${leading}request if ${condition}${enableCondition} then ${actionText}`,
      converted: true,
    };
  }
  ...
  return {
    line: `${leading}${converted.phase} if \${url} ~= ${regexLiteral(
      pattern,
      'i',
    )}${enable.conditions.length ? ` && ${enable.conditions.join(' && ')}` : ''} then ${converted.actions.join(' | ')}`,
    converted: true,
  };
}
...
export function convertLegacyRewrite(
  ...
  lines.forEach((line, index) => {
    ...
    const converted = convertLegacyLine(
      line,
      lineNumber,
      captureName,
      argumentNames,
