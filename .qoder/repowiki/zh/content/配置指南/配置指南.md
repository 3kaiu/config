...
## 更新摘要
**所做更改**
- 移除了对集中式 Surge.conf 配置的引用，反映新的模板化系统架构
- 更新了项目结构说明，强调模板驱动的配置生成机制
- 修正了 Profile 目录中的配置文件列表，移除已删除的 Surge.conf
- 更新了片段模板的文件扩展名，从 .tpl 改为 .qx 格式
- 强化了模板化系统的技术细节和最佳实践
...
**重要更新**：Surgio 已从集中式配置文件模式完全迁移到基于模板的架构，不再包含集中的 Surge.conf 配置文件，所有平台配置均通过模板系统动态生成。

## 项目结构
Surgio 采用"统一配置 + 模板化输出"的组织方式：
...
- 常见片段
  - ai-services.qx：AI 服务相关规则与脚本。
  - bank-ad-reject.qx：银行类应用广告拦截。
  - developer.qx：开发者工具与调试增强。
  - social.qx：社交平台优化。
  - streaming.qx：流媒体加速与去广告。
- 使用方式
  - 在主模板中按条件引入片段。
  - 通过参数化配置控制行为。
...
- 文件
  ...
  - **注意**：Surge.conf 已不再包含在 Profile 目录中，Surge 配置现通过 surge.tpl 模板动态生成。
- 注意事项
  - 不要手动修改，以免与源码不一致。
...
Conf --> OutS["Profile/Surge.conf (动态生成)"]
...
## 结论
Surgio 通过统一配置与模板化渲染，实现了跨平台配置的一致性与可维护性。从集中式配置向模板化架构的迁移，进一步提升了系统的灵活性和可扩展性。遵循本文的结构与最佳实践，可以高效地管理多平台规则、脚本与插件，并在构建期保障质量。结合迁移与兼容性说明，可在版本演进中平滑升级。

## 附录

### 多平台配置格式与语法要点
- Surge
  - 配置文件：通过 surge.tpl 模板动态生成，不再包含静态 Surge.conf 文件。
...
- 迁移指南
  ...
  - 特别注意：集中式 Surge.conf 配置已被移除，所有 Surge 配置现通过模板系统生成。
- 兼容性
  - 保持向后兼容，提供过渡期与弃用警告。
- 建议
  - 分阶段迁移，先在测试环境验证。
