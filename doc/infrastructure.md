...
> ⚠️ 域名过期被抢注 = 全量 script-path 可被第三方投毒（52 个插件外壳 + `Profile/Loon.lcf` 引用面同时受影响 — 2026-09-20 复核: 自 QX/免费代理移除后发布面收敛为 Loon.lcf 单 Profile）。
> 建议：开启自动续费 + 注册商到期邮件 + 日历提醒（到期前 30 天）。

## 3. 完整性保障

- `mirror-scripts.yml`：每日 03:00 从上游抓取（2026-09-24 复核：48 个 `mirror()` 声明，循环展开 53 个目标 → `Mirror/MANIFEST.json` 53 条目），经四重门禁（`*.js` 语法 / 体积下限 = 绝对 200B **+ 相对上次原始抓取 50%** / 非 HTML / `.plugin` script-path 域白名单）后写入 `Mirror/`，**走 PR 人工审核**合并（不再直推 main）。插件外壳远程引用已收敛到 `ws.wenn.in/main/Mirror/`（插件内部 bundle 引用仍直连上游，见 3b）。
...
1. **kelee.one 1 个 `.lpx`**（`iRingo.WeatherKit.lpx`）：Cloudflare Turnstile 阻挡自动抓取，无法镜像/校验，Loon 端直接从该站加载。介意者在 Loon 内停用对应插件。
