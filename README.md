...
### 🚨 3.6 节点故障应急 (Failover Runbook)

当前为**单节点架构**（东京），策略组刻意不放 DIRECT 自动回落以防流量泄漏，代价是节点挂掉时代理流量全断。故障处置：

1.  **感知**：`health-notify.js` 每 6 小时检测一次，失败时本地通知 + Bark/Telegram（如已配置），通知正文含应急指引。
2.  **临时恢复**：将 **Final 策略组手动切换为 DIRECT**（Loon/QX/Surge 三端通用）——未命中分流规则的流量恢复直连；国内流量本就走 DIRECT 规则不受影响，仅海外访问降级为直连（可能受限）。
3.  **修复后**：切回 Proxy，手动跑一次健康检测确认恢复。
4.  **根治方向**：在订阅（Sub-Store / ws.wenn.in 集合）中加入**第二个节点**，`Proxy` url-test 组会自动双节点容灾（策略组正则 `.*` / `.` 会自动纳管新节点，无需改配置）。
