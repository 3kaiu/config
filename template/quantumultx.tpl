...
https://ws.wenn.in/main/QX/apple/TV.conf, tag=🍎TV增强, update-interval=86400, enabled=true
https://ws.wenn.in/main/QX/apple/LocationService.conf, tag=🍎定位服务增强, update-interval=86400, enabled=true
https://ws.wenn.in/main/QX/apple/Search.conf, tag=🍎搜索建议增强, update-interval=86400, enabled=true
...
# 🍿️ DualSubs YouTube 双语字幕
^https?:\/\/(www|m|tv)\.youtube\.com\/youtubei\/v1\/player(\?.+)?$ url script-request-body https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.request.bundle.js
^https?:\/\/(www|m|tv)\.youtube\.com\/youtubei\/v1\/player(\?.+)?$ url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.response.bundle.js
^https?:\/\/youtubei\.googleapis\.com\/youtubei\/v1\/player(\?.+)?$ url script-request-body https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.request.bundle.js
^https?:\/\/youtubei\.googleapis\.com\/youtubei\/v1\/player(\?.+)?$ url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.response.bundle.js
^https?:\/\/youtubei\.googleapis\.com\/youtubei\/v1\/get_watch(\?.+)?$ url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.response.bundle.js
^https?:\/\/(www|m)\.youtube\.com\/api\/timedtext(\?.+)?$ url script-request-header https://ws.wenn.in/main/Mirror/dualsubs/DualSubs.YouTube.request.bundle.js
^https?:\/\/(www|m)\.youtube\.com\/api\/timedtext\?(.*)subtype=(Official|External) url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/Composite.Subtitles.response.bundle.js
^https?:\/\/(www|m)\.youtube\.com\/api\/timedtext\?(.*)subtype=Translate url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/Translate.response.bundle.js
# 🍿️ DualSubs Netflix 字幕翻译
^https?:\/\/(.+)\.oca\.nflxvideo\.net\/\?o=\d+&v=\d+&e=\d+&t=.+ url script-response-body https://ws.wenn.in/main/Mirror/dualsubs/Translate.response.bundle.js
...
^https:\/\/duckduckgo\.com\/\?q=qm\+([^&]+).+ url 302 https://www.qimai.cn/search/index/country/cn/search/$1
^https:\/\/duckduckgo\.com\/\?q=gh\+([^&]+).+ url 302 https://github.com/search?q=$1
^https:\/\/duckduckgo\.com\/\?q=([^&]+).+ url 302 https://www.google.com/search?q=$1

[task_local]
# 与 Loon 端 cron 对齐 (02:11)。⚠️ QX task_local 受宿主超时限制 (远短于 Loon),
# 高阶任务 (激励×9+每日×5, 每步 5-10s 等待, 全量需 2-3 分钟) 在 QX 端大概率被截断,
# 以是否收到"每日任务结果"汇总通知为准; 需要完整执行建议走 Loon 端。
