...
- `npm test` — 225 个行为级用例(含 `tools/` 单测),引用全部 27 个 Scripts 产物
...
| `test/cases/*.test.js` | 225 用例 | 新增/改脚本须补用例;响应类脚本用 `a.doneCalled(state)` 断言 `$done` 被调用;`.mjs` 工具用动态 `import()` 引入(勿用 `require(esm)`,会无谓抬高 engines 下限) |
...
- [Rule] 480 行(2026-09-18 实测,主 [Rule] 段非注释行;2026-09-11 精简审计删 3 条同策略死规则: auth0.openai.com/nrdns.netflix.com/calls.signal.org);GEOIP 顺序已修;AdBlock 域硬拦截已覆盖规则
